...
# RLLAB Lance v2.0 One-Shot Plan

이 문서는 v1 → v2 일괄 승격을 결정할 때 사용한 **계획서/근거 문서**입니다.
실제 진행 트래커는 [`V2_IMPLEMENTATION_CHECKLIST.md`](V2_IMPLEMENTATION_CHECKLIST.md)
이고, 항목별 `[x]/[~]/[ ]` 상태는 그쪽에 살아있습니다. 여기서는 v2 contract가
어떤 항목을 유지하고/제거하고/하지-않기로 했는지를 평문으로 기술합니다.
...
* `video_blob`는 `lance.blob.v2`.
* `data_storage_version >= "2.2"`.
* `lance.blob_field("video_blob")`.
* `lance.blob_array(...)`.
* external URI blob 금지.
* inline bytes only.
...
* `frames.observation_state`: `fixed_size_list<float32, state_dim>`.
* `frames.action`: `fixed_size_list<float32, action_dim>`.
* `episodes.observation_state`: `large_list<fixed_size_list<float32, state_dim>>`.
* `episodes.actions`: `large_list<fixed_size_list<float32, action_dim>>`.
* 16D / 19D silent mixing 금지.
...
* magic prefix 사용.
* length/state_dim/action_dim 포함.
* timestamp float64 LE.
* state/action float32 LE.
* NaN / Inf hard-reject.
...
* `task_segments.start_frame`.
* `task_segments.end_frame_exclusive`.
...
* `episode_index`, `frame_index`, `global_frame_index`, `media_id` → BTREE.
* `task_index`, `camera_id`, `is_bad_frame`, `split`, `source_dataset`, `session_id`, `embodiment_id` → BITMAP.
* `indexes.created`와 `indexes.recommended` 분리.
...
* `state_norm`, `action_norm`은 optional.
* `is_bad_frame`은 required non-null.
* `quality_flags`는 v2 mandatory 아님.
* `quality_score`는 v2 mandatory 아님.
* `timestamp_ns`는 v2 mandatory 아님.
...
* `actions.action.body.semantics` block exists.
* required keys:
...
* Blob v2 유지.
* FixedSizeList 유지.
* binary trajectory hash 유지.
* half-open task segments 유지.
* typed indexes 유지.
...
* `timestamp_ns` mandatory.
* `quality_flags` mandatory.
* `quality_score` mandatory.
* `meta/embodiments.json` mandatory.
* `meta/calibration/cameras.json` mandatory.
* `primary_access_patterns` mandatory.
* keyframes table mandatory.
* external MP4 primary storage.
* in-place v1 → v2 migration CLI.
